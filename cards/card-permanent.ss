...
(library
 ...
 (define-dispatch-subclass (card-permanent name color cost game player picture)
   (play destroy turn-begin phase-begin phase-end turn-end can-play? supports-type? get-type changed-zone create-card-virtual-perm-via-stack)
   ...
   (define act-play (card-action game
                                 ...
                                 (lambda ()
                                   (this 'can-play?))
                                 (lambda ()
                                   (((game 'get-field) 'get-stack-zone) 'push! (this 'create-card-virtual-perm-via-stack))
                                   (set! already-going-to-play #t))))
                                        
   (define (create-card-virtual-perm-via-stack)
     (card-virtual-perm-via-stack this))
   (define (play)
   ...
   (define (changed-zone zone)
     ...
     (if (eq? ((player 'get-field) 'get-in-play-zone) zone)
         (this 'play)))
